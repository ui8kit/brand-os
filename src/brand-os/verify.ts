...
import { BrandOsManifest } from './emitter.js';
...
export interface VerificationIssue { code: string; path: string; message: string; }
export interface BundleVerification { ok: boolean; issues: VerificationIssue[]; manifest?: BrandOsManifest; }
...
export function verifyDesignMd(content: string): VerificationIssue[] {
...
function parseManifest(bundleDir: string, issues: VerificationIssue[]): BrandOsManifest | undefined {
  ...
  return value as unknown as BrandOsManifest;
}

export function verifyBundle(schema: BrandOsSchema, bundleDir: string, strict = false): BundleVerification {
  const issues: VerificationIssue[] = [];
  ...
  for (const file of manifest.files) {
    ...
    let absolute: string;
    try { absolute = resolveContainedPath(bundleDir, file.path, 'manifest file path'); }
    catch (error) {
    ...
    if (statSync(absolute).size !== file.bytes) issues.push({ code: 'size_mismatch', path: `/${file.path}`, message: 'File size differs from the manifest.' });
    if (sha256File(absolute) !== file.sha256) issues.push({ code: 'checksum_mismatch', path: `/${file.path}`, message: 'File checksum differs from the manifest.' });
  }
  const designPath = join(bundleDir, 'DESIGN.md');
  if (existsSync(designPath)) issues.push(...verifyDesignMd(readFileSync(designPath, 'utf8')));
  else issues.push({ code: 'missing_design_md', path: '/DESIGN.md', message: 'DESIGN.md is missing.' });
  if (strict) {
    for (const file of listFilesRecursive(bundleDir)) {
      if (!declared.has(file)) issues.push({ code: 'undeclared_file', path: `/${file}`, message: 'File is not declared by the manifest.' });
    }
  }
