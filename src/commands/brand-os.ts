...
import { BrandOsSchema, PromptPack, PromptPackSurface } from '../brand-os/types.js';
...
function buildPromptPack(schema: BrandOsSchema): PromptPack {
  const surfaces: Record<string, PromptPackSurface> = {};
...
export async function runBrandOs(args: BrandOsCliArgs): Promise<number> {
  ...
  if (args.strict && (contrastWarnings.length > 0 || qualitySignals.length > 0)) {
    ...
    if (args.json) console.log(JSON.stringify(result, null, 2));
      ...
      for (const warning of contrastWarnings) console.error(`  [warning] ${warning}`);
      for (const finding of qualitySignals) console.error(`  [warning] ${finding.code} ${finding.path}: ${finding.message}`);
    }
    return 1;
  }
