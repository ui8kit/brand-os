...
async function main() {
    ...
    try {
        ...
        if (wantsJson)
            console.log(JSON.stringify({ ok: false, apiVersion: '1', code: 'usage_error', message }, null, 2));
        else
            console.error(`brand-os: ${message}\n\n${usage()}`);
        return 2;
    }
    if (args.mode === 'help') {
        console.log(usage());
    ...
    if (args.mode === 'version') {
        console.log(args.json ? JSON.stringify({ ok: true, name: 'brand-os', version: CLI_VERSION }) : CLI_VERSION);
        return 0;
    }
    if (args.help) {
    ...
    try {
        ...
        if (error instanceof SchemaValidationError) {
            ...
            if (args.json)
                ...
                console.error(`brand-os: ${error.message}`);
        ...
        if (args.json)
            console.log(JSON.stringify({ ok: false, apiVersion: '1', code: 'runtime_error', message }, null, 2));
        else
            console.error(`brand-os: ${message}`);
        return 3;
    }
