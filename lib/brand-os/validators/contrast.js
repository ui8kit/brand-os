...
export function validateThemeContrast(schema) {
    const warnings = [];
    ...
    for (const [themeName, theme] of Object.entries(themes)) {
        ...
        if (bodyRatio === null) {
            ...
            warnings.push(`Theme "${themeName}" body text contrast ${formatRatio(bodyRatio)} is below WCAG 2.2 threshold ${budget.minBodyAA}:1.`);
        }
        ...
        if (largeRatio === null) {
            ...
            warnings.push(`Theme "${themeName}" primary-on-primary contrast ${formatRatio(largeRatio)} is below WCAG 2.2 threshold ${budget.minLargeAA}:1.`);
        }
        if (theme.base === 'dark') {
            ...
            if (headingRatio === null) {
                ...
                warnings.push(`Theme "${themeName}" dark heading contrast ${formatRatio(headingRatio)} is below configured threshold ${budget.minHeadingOnDark}:1.`);
            }
        }
    }
    return warnings;
